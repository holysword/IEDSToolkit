...
namespace IEDSToolkit
{
    class PrintListView
    {
        ...
        private bool InitPrintPage(out Margins margins)
        {
            margins = null;
            /* 获取当前 listview 的分辨率 */
            Graphics g = myListView.CreateGraphics();
            float x = g.DpiX;
            g.Dispose();
            /* 显示纸张设置对话框 */
            ...
            {
                listViewWidth += myListView.Columns[i].Width;
            }
        ...
        private bool InitPrintPageNumber(Margins margins)
        {
            ...
            if (firstPageItem >= myListView.Items.Count)
            {   // 需打印的元素只有一页    
                m_nPageCount = 1;
            }
            else
            {  // 需要打印的元素有多页    
                ...
                int leftItems = myListView.Items.Count - firstPageItem;
        ...
        protected virtual void PrintPageBody(Graphics g)
        {
            ...
            for (int i = 0; (i < printItemPerPage) && (m_nCurPrintItem < myListView.Items.Count); i++)
            {
        ...
        private void DrawColumnHeader(Graphics g, Font f, Brush b, Rectangle rect)
        {
            ...
            for (int i = 0; i < myListView.Columns.Count; i++)
            {
                itemSize = new Size(myListView.Columns[i].Width, rect.Height);
                itemRect = new Rectangle(location, itemSize);
                //g.DrawRectangle(defaultPen, itemRect);
                g.DrawString(myListView.Columns[i].Text, f, b, itemRect, sf);
                location.X += myListView.Columns[i].Width;
            }
        }
        /// <summary>    
        /// 打印 listview 的 item    
        ...
        private void DrawItem(Graphics g, Font f, Brush b, Rectangle rect)
        {
            ...
            for (int i = 0; i < myListView.Columns.Count; i++)
            {
                ...
                itemRect = new Rectangle(location, itemSize);
                //g.DrawRectangle(defaultPen, itemRect);
                g.DrawString(myListView.Items[m_nCurPrintItem].SubItems[i].Text, f, b, itemRect, sf);
                location.X += myListView.Columns[i].Width;
            }
        }
    }
